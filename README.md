...
7) Что бы запустить тест нужно прописать такие команду:
``` bash
gradle test
``` 
или
...
- Для запуска нужно сначала создать контейнеры в докере для баз данных(пункт 6 Запуска проекта), потом можно запускать приложение или тесты.
